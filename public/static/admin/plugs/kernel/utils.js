define(["jquery", "tableSelect", "ckeditor"], function ($, tableSelect, undefined) {
    var {form, layer, table, laydate, upload, element, laytpl, tableSelect, util} = layui,
    ...
    var admin = {
            ...
            ajax: function (type, option, ok, no, ex) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        var code = eval('res.' + option.statusName)
        ...
        common: {
            ...
            // 时间戳转日期
            timeToDate: function (unixTime, format, timeZone) {
                if (typeof (timeZone) == 'number') {
                    unixTime = parseInt(unixTime) + parseInt(timeZone) * 60 * 60;
                }
                unixTime = util.toDateString(unixTime * 1000, format || 'yyyy-MM-dd HH:mm:ss')
                return unixTime
            },
            // 地区三级联动
            linkage: function (operate = {}) {
                var provinceAttr = operate.province || 'province',
                    cityAttr = operate.city || 'city',
                    areaAttr = operate.area || 'area',
                    url = operate.url || '/admin/common/get_area'

                form.on('select(' + provinceAttr + ')', function (data) {
                    $(`.${cityAttr}`).empty().html('<option value="">请选择市</option>')
                    $(`.${areaAttr}`).empty().html('<option value="">请选择县/区</option>')
                    admin.request.get({url: url + '?pid=' + data.value}, function (res) {
                        res.data.forEach(v => {
                            $(`.${cityAttr}`).append(`<option value="${v.id}">${v.ext_name}</option>`)
                        })
                        form.render()
                    })
                })
                form.on('select(' + cityAttr + ')', function (data) {
                    $(`.${areaAttr}`).empty().html('<option value="">请选择县/区</option>')
                    admin.request.get({url: url + '?pid=' + data.value}, function (res) {
                        res.data.forEach(v => {
                            $(`.${areaAttr}`).append(`<option value="${v.id}">${v.ext_name}</option>`)
                        })
                        form.render()
                    })
                })
        ...
        table: {
            render: function (options) {
                ...
                // 初始化表格左上方工具栏
                // ['refresh', 'add', 'delete', 'export']
                options.toolbar = options.toolbar || ['refresh', 'add'];
                        ...
                        switch (d.search) {
                            ...
                            case 'select':
                                ...
                                formHtml += '\t<div class="layui-form-item layui-inline">\n' +
                                    '<label class="layui-form-label">' + d.title + '</label>\n' +
                                    '<div class="layui-input-inline">\n' +
                                    '<select class="layui-select" id="c-' + d.fieldAlias + '" name="' + d.fieldAlias + '"  data-search-op="' + d.searchOp + '" ' + d.selectAttribute + '>\n' +
                            ...
                            case 'batch':
                                ...
                                formHtml += '\t<div class="layui-form-item layui-form-text">\n' +
                                    ...
                                    '<textarea id="c-' + d.fieldAlias + '" name="' + d.fieldAlias + '"  data-search-op="' + d.searchOp + '"  value="' + d.searchValue + '" placeholder="' + d.searchTip + '" class="layui-textarea"></textarea>\n' +
                                    '</div>\n' +
                                    '</div>';
                                break;
                        }
            ...
            buildOperatHtml: function (operat) {
                ...
                if (operat.method === 'open') {
                    ...
                    // 新增post请求方法
                    formatOperat.method = formatOperat.method !== '' ? 'data-request="' + formatOperat.url + '" data-title="' + formatOperat.title + '" data-table="' + operat.tableId + '" data-field="' + operat.field + '" data-pk="' + operat.pk + '" ' : '';
                }
            ...
            image: function (data, option) {
                ...
                if (value === undefined || value === null) {
                    return '<img alt="" style="max-width: ' + option.imageWidth + 'px; max-height: ' + option.imageHeight + 'px;" src="' + value + '" data-image="' + title + '">';
                } else {
                    var values = value.split(option.imageSplit),
                        valuesHtml = [];
                    values.forEach((value, index) => {
                        valuesHtml.push('<img alt="" style="max-width: ' + option.imageWidth + 'px; max-height: ' + option.imageHeight + 'px;" src="' + value + '" data-image="' + title + '">');
                    });
            ...
            switch: function (data, option) {
                ...
                option.tips = option.tips || '开启|禁用';
                option.pk = option.init.pk;
                option.id = eval('data.' + option.pk)
                try {
                    var value = eval("data." + field);
                } catch (e) {
                    var value = undefined;
                ...
                return laytpl('<input type="checkbox" name="' + option.field + '" value="' + option.id + '" lay-skin="switch" lay-text="' + option.tips + '" lay-filter="' + option.filter + '" ' + checked + ' >').render(data);
            },
        ...
        // 搜索框条件监听
        getSelectList: function (table, field = 'name', where = []) {
            var mydata;
            $.ajax({
                url: '/admin/common/getSelect',
                data: {table: table, field: field, where: where},
                type: 'post',
                contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                async: false,
                dataType: 'json',
                ...
                if (uploadList.length > 0) {
                    $.each(uploadList, function (i, v) {
                        ...
                        upload.render({
                            ...
                            accept: 'file',
                            exts: exts,
                            data: {type: uploadType || 'file'},
                            // 让多图上传模式下支持多选操作
                            multiple: uploadNumber !== 'one',
                            done: function (res) {
                                if (res.code === 0 || res.code === 1) {
                                    var view_url = res.data.view_url,
                                        url = res.data.url
                                    if (uploadNumber !== 'one') {
                                        var oldUrl = $(elem).val();
                                        if (oldUrl !== '') {
                                    ...
                                    $(elemView).val(view_url)
                                    $(elem).val(url)
                                    $(elem).trigger("input");
                                    $(elemView).trigger("input");
                ...
                if (uploadSelectList.length > 0) {
                    $.each(uploadSelectList, function (i, v) {
                        ...
                        tableSelect.render({
                            ...
                            searchType: 'more',
                            searchList: [
                                {searchKey: 'title', searchPlaceholder: '请输入文件类型'},
                            ],
                            table: {
                                url: admin.url('common/get_file_manage'),
                                cols: [[
                                    {type: selectCheck},
                                    ...
                                    {field: 'type', width: 150, title: '图片类型', align: "center"},
                                ...
                                $.each(data.data, function (index, val) {
                                    ...
                                    viewUrlArray.push(val.view_url)
                                });
                                var url = urlArray.join(uploadSign),
                                    viewUrl = viewUrlArray.join(uploadSign)
                                admin.msg.success('选择成功', function () {
                                    $(elem).val(url);
                                    $(elemView).val(viewUrl)
                                    $(elem).trigger("input");
                                });
                            }
                        })
                ...
                if (editorList.length > 0) {
                    $.each(editorList, function (i, v) {
                        CKEDITOR.replace(
                            ...
                            {
                                height: $(this).height(),
                                filebrowserImageUploadUrl: '/api/common/uploadEditor',
                            });
                    });
                }
            },
